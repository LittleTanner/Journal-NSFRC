...
class EntryListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let entry = EntryController.sharedInstance.fetchedResultsController.fetchedObjects?[indexPath.row],
            let entryDate = entry.timestamp  else { return UITableViewCell() }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "toDetailView" {
            ...
            guard let entry = EntryController.sharedInstance.fetchedResultsController.fetchedObjects?[selectedIndexPath.row] else { return }
